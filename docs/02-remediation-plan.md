...
| [D19](01-defect-analysis.md#d19) | POST /products/batch binds an inline structural type, so validation is skipped and the real error is masked | `curl -i -s -X POST localhost:3000/products/batch -H 'content-type: application/json' -d '{}'` must return 400 with `productIds must be an array` (currently 400 'Batch processing failed'). `curl -i -s -X POST localhost:3000/products/batch -H 'content-type: application/json' -d '{"productIds":"1,2,3"}'` must return 400 naming the type error. `'{"productIds":[]}'` must still return 201 with `processed: 0` — [04](04-scope.md) records `@ArrayNotEmpty` and `@ArrayMaxSize` as removed, so rejecting an empty or large batch here is a failure, not a pass. |
